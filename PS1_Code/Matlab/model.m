...
classdef model
    methods(Static)
        ...
        function par = setup()            
            ...
            %% Rate of return
            par.r = 0.05; % Rate of return on capital
            par.w = 2.5; % Wage rate 
            par.n = 1; % Inelastic labor supply

            assert(par.r  > 0 && par.r  < 1.00,'Rate of return on capital should be between 0 and 1.\n')
            assert(par.n==1,'Labor supply should be inelastic.\n')

            %% Tax rates and government spendings
            %%% Taxes
            par.tauk = 0.5; % Tax rate for capital
            par.taun = 0.3; % Tax rate for labor
