...
%{
    model2.m
    -------
    This code sets up the model with a discrete investment choice.
%}
...
classdef model2
    methods(Static)
        ...
        function par = setup()            
            ...
            par.kappa = 0.4; % Fixed cost of investment.
            par.lambda = 0.8; % Fraction of revenue retained when investing.
            ...
            assert(par.kappa >= 0.0 && par.kappa <= 1.0,'Fixed investment cost must be from 0 to 1.\n')
            assert(par.lambda >= 0.0 && par.lambda <= 1.0,'Fraction of revenue loss must be from 0 to 1.\n')
        ...
        function par = gen_grids(par)
            ...
            par.klen = 300; % Grid size for capital.
            par.kmax = 30.0; % Upper bound for capital.
            par.kmin = 1e-4; % Minimum capital.
            ...
            par.kgrid = linspace(par.kmin,par.kmax,par.klen)'; % Equally spaced, linear grid for capital.
                
            %% Discretized productivity process.
                  
            par.Alen = 7; % Grid size for productivity.
        ...
        function [y,pi] = tauchen(mu,rho,sigma,N,m)
            ...
            ar_mean = mu/(1-rho); % The mean of a stationary AR(1) process.
            ar_sd = sigma/((1-rho^2)^(1/2)); % The std. dev of a stationary AR(1) process.
	        ...
	        pi = normcdf(ymatk,ymatj - (d/2),sigma) - normcdf(ymatk,ymatj + (d/2),sigma); % Transition probabilities.
	        pi(:,1) = normcdf(y(1), mu + rho*y - (d/2), sigma); % Transition to state 1.
	        pi(:,N) = 1 - normcdf(y(N), mu + rho*y + (d/2), sigma); % Transition to state N.
        ...
        function cost = total_cost( k, par)
            ...
            if invest > 0
                cost = par.kappa * k; % Fixed cost of investing.
            else
                cost = 0;
            end
        end
