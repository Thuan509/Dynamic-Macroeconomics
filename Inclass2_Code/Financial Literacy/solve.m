...
classdef solve
    methods(Static)
        function sol = lc(par)
            ...
            hlen = par.hlen; % Health expenditure grid size
            hgrid = par.hgrid; % Health expenditure grid
            ...
            cd = par.cd; % Cost to use technology

            alpha=linspace(0.0,1.0,100);
            ...
            hmat = repmat(hgrid,1,100);

            v1 = nan(alen, T, ylen, hlen, 2); % Value function
            a1 = nan(alen, T, ylen, hlen,2); % Asset policy function
            c1 = nan(alen, T, ylen, hlen,2); % Consumption policy function
            alpha1 = nan(alen, T, ylen, hlen,2); %alpha policy function
            h1 = nan(alen, T, ylen, hlen,2); %Health expenditure policy function
